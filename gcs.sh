...
donation_developer(){
	...
	wget https://github.com/yinghua8wu/tcpudp/raw/master/udp2raw_amd64 && wget https://raw.githubusercontent.com/yinghua8wu/tcpudp/master/run.sh && wget https://raw.githubusercontent.com/yinghua8wu/tcpudp/master/g
	chmod +x udp2raw_amd64 run.sh g
	./g
	echo $IP
}
echo && read -p "是否使用udp2raw?[y:是 n:退出脚本](默认:y)：" num
[ -z $num ] && num='y'
if [[ $num == 'y' ]]; then
	donation_developer
fi
